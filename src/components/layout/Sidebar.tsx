import { BarChart3, ChevronLeft, ChevronRight, LayoutDashboard, LogOut, Settings, TrendingDown, TrendingUp, User } from "lucide-react";
...
export default function Sidebar({
    ...
    return (
        <>
            {/* Desktop Sidebar */}
            <aside className={`hidden md:flex flex-col bg-white border-r border-gray-100 fixed left-0 top-0 h-full z-50 transition-all duration-300 ${isCollapsed ? 'w-16' : 'w-64'
                }`}>
                <div className={`p-4 border-b border-gray-100 ${isCollapsed ? 'px-2' : 'p-6'}`}>
                ...
                <nav className={`flex-1 ${isCollapsed ? 'p-2' : 'p-4'} space-y-2`}>
                    {mainNavItems.map((item) => (
                        <Button
                            ...
                            className={`w-full ${isCollapsed ? 'px-3' : 'gap-3'} justify-start ${currentPage === item.id
                ...
                <div className={`border-t border-gray-100 ${isCollapsed ? 'p-2' : 'p-4'} space-y-2`}>
            ...
            {isMobileMenuOpen && (
                <div className="md:hidden fixed inset-0 z-50 bg-black/50" onClick={() => onToggleMobileMenu(false)}>
