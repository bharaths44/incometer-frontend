...
import { useCategoryBreakdown, useBudgetAnalytics, useExpenseSummary } from "../hooks/useAnalytics";

export default function Analytics() {
    const userId = 1; // TODO: Get from auth context

    const { data: categoryBreakdown = [], isLoading: categoryLoading } = useCategoryBreakdown(userId);
    const { data: expenseSummary, isLoading: expenseLoading } = useExpenseSummary(userId);
    const { data: budgetAnalytics = [], isLoading: budgetLoading } = useBudgetAnalytics(userId);
