...
export const useUpdateCategory = () => {
    ...
    return useMutation({
        mutationFn: ({ id, category }: { id: number; category: CategoryRequestDTO }) => updateCategory(id, category),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: categoryKeys.lists() });
        },
    });
...
export const useDeleteCategory = () => {
    ...
    return useMutation({
        mutationFn: ({ id, userId }: { id: number; userId: number }) => deleteCategory(id, userId),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: categoryKeys.lists() });
        },
    });
